...
def heapify(A, i, n=None):
    ...
    rightChild = right(i)
    smallestIndex = i
    """Turns out & != AND that was a HUGE problem for me"""
    if leftChild < n and A[leftChild] < A[smallestIndex]:
        smallestIndex = leftChild
    if rightChild < n and A[rightChild] < A[smallestIndex]:
        smallestIndex = rightChild
    if A[i] > A[smallestIndex]:
        swap(A, i, smallestIndex)
        heapify(A, smallestIndex)
...
def buildHeap(A):
    ...
    """ Had 46 heapify calls for a while, then I realized I was trying to heap leaves for no reason
    So this gets just the indexes with leaves, hopefully its not a problem """
    startIndex = len(A) // 2 - 1
...
def heapInsert(A, v):
    ...
    A.append(v)
    indexOfNewElement = len(A) - 1
    parentIndex = parent(indexOfNewElement)
    while A[indexOfNewElement] < A[parentIndex] and indexOfNewElement != 0:
        swap(A, indexOfNewElement, parentIndex)
