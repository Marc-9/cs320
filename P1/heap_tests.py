...
def report_counts_on_basic_ops(A, expected_build_count, expected_extract_count, expected_insert_count):
    ...
    print("buildHeap heapify calls within 30% of expected number of calls: {}".format(
        within_30_percent(expected_build_count, bh)))

    heap.reset_counts()
    m = heap.heapExtractMin(A)
    ex = heap.current_counts()['heapify_call_count']
    print("heapExtractMin heapify calls within 30% of expected number of calls: {}".format(
        within_30_percent(expected_extract_count, ex)))

    heap.reset_counts()
    heap.heapInsert(A, m)
    ins = heap.current_counts()['swap_count']
    print("heapInsert swap calls within 30% of expected number of calls: {}".format(
        within_30_percent(expected_insert_count, ins)))
